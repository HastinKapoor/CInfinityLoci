...
#check (_ : EuclideanSpace ℝ (Fin n))
...
def RtoR1 (x : ℝ) : Fin 1 → ℝ :=
  fun (_ : Fin 1) => x

theorem RtoR1_ContDiff : ContDiff ℝ ⊤ RtoR1 := by
  sorry

def ArraytoRn (x : Array (ℝ^n)) : Fin x.size → ℝ^n :=
  fun (y : Fin x.size) => x[y]

def HadamardsLemma_Function (x y : ℝ^1) (f : C^∞(ℝ^1)) :=


lemma HadamardsLemma {x : ℝ^1} (f : C^∞(ℝ^1)) : ∃ (g : C^∞(ℝ^1)), f x - f y = RtoR1 (Matrix.dotProduct (x - y)  (g (y))) := by
  use (fun (z : ℝ^1) => Matrix.dotProduct z z) -- example function
...
instance [CinftyRing A] (I: Ideal A) : CinftyRing (A ⧸ I) where
  intrprt := by
    sorry
  fnctr := by
    sorry
